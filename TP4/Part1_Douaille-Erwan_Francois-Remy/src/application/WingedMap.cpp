...
void WingedMap::read(const string &nom,Winged *winged) {
  ...
  for(unsigned int i=0;i<_mesh->nbPosition();i++) {
    _vertexEdge[i].clear();
    _wvertex.push_back(winged->createVertex(_mesh->position(i)));
  }
  ...
  for(unsigned int i=0;i<_mesh->nbFace();i++) {
    ...
    for(unsigned int j=0;j<_mesh->nbVertex(i);j++) {
      ...
      unsigned int v=_mesh->indexPosition(i,j);

      wv_begin=_wvertex[v];


      // On cherche et on crée éventuellement une nouvelle arête
      unsigned int next=_mesh->indexPosition(i,j+1);
    ...
    current0=_mesh->indexPosition(i,0);
    current1=_mesh->indexPosition(i,1);
    pred=_mesh->indexPosition(i,-1);
    succ=_mesh->indexPosition(i,2);;
    ...
    for(unsigned int j=0;j<_mesh->nbVertex(i);j++) {
      ...
      current0=current1;
      current1=succ;
      succ=_mesh->indexPosition(i,j+3);
      prede=currente;
      currente=succe;
      succe=this->findEdge(_wvertex[current1],_wvertex[succ]);
